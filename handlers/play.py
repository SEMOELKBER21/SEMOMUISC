...
@Client.on_message(command(["player", f"player@{BOT_USERNAME}", "تحكم"]) & filters.group & ~filters.edited)
...
async def m_cb(b, cb):
    ...
    if type_ == "pause":
        ...
        if chet_id in callsmusic.pytgcalls.active_calls:
            ...
            callsmusic.pytgcalls.leave_group_call(chet_id)
            await cb.message.edit("⏹ **تم ايقاف التشغيل بنجاااح ........!**")
        else:
            await cb.answer("الحســاب المساعــد  لم يتصــل بعــد!", show_alert=True)


@Client.on_message(command(["play", "نقيب", "سيمو", "شغل", "تشغيل"]) & other_filters)
            ...
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await USER.send_message(
                        message.chat.id, "انا الحســاب المساعــد لبوت تشغيل الاغاني" 
                    )
                    await lel.edit(
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("SUPPORT", url=f"https://t.me/semoelkber1")
                ],
            ]
        )
